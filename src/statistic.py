...
def plots(query, tmap, small_intron, statistic):
    ...
    fig = px.bar(counts2, x=counts2['group'], y=counts2['count'],
    labels = {'count': 'LncRNA transcripts number', 'group': 'LncRNA classes'}, height = 400)
    st.plotly_chart(fig)


# exon structure of lncRNA
    chart = query[query[7].str.contains("exon")]
    spl = chart[3].str.split('.', expand=True)
...
def value_prepare(x, z, df):
    name = x[0].str.rsplit('_', n=1, expand=True)
    x['srx'] = name[1]
    x = x[[0, 'srx']]
    same = pd.merge(df, x, how='inner', left_on=[1], right_on=['srx'])
    pr = same[2].value_counts().reset_index()
    pr = pr.rename(columns={2: 'index'}, inplace=False)
    pr = pr.rename(columns={'count': z}, inplace=False)
    return (pr, same)
...
def tissue(blast, old_new, df, org, org_id, out):
    ...
    df_tis = df_tis.rename(columns={2: 'index'}, inplace=False)
    ...
    full_table = pd.merge(transc, transc_cod, how='inner', left_on=['index'], right_on=['index'])
    full_table = pd.merge(full_table, transc_non, how='inner', left_on=['index'], right_on=['index'])
    full_table = pd.merge(full_table, df_tis, how='inner', left_on=['index'], right_on=['index'])
    full_table = full_table.fillna(0)

    full_table['con'] = full_table['Conserved'] / full_table['count']
    full_table['noncon'] = full_table['Nonconserved'] / full_table['count']
    full_table['cod'] = full_table['Coding'] / full_table['count']
