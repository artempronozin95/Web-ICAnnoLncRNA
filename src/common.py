...
def page_setup(page: str = "") -> dict[str, Any]:
    ...
    Path(st.session_state.workspace,
         "Upload_files").mkdir(parents=True, exist_ok=True)
...
⚠️ Anyone with this ID can access your data!"""
        ...
        if page == "workflow":
            st.markdown("📁 **Upload_files**")
            st.multiselect("Upload_files",  options=[Path(f).stem for f in Path(st.session_state.workspace, "Upload_files").glob("*.fasta")],
                           default=params["selected-files"], key="selected-files", label_visibility="collapsed")
